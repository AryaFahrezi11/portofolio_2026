...
import { getAboutContent, getHomeStats, updateAboutContent, updateHomeStats } from '@/app/actions/portfolio';
...
export default function AdminAbout() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (aboutData) setAboutContent(aboutData as unknown as AboutContent);
      if (statsData) setStats(statsData as unknown as HomeStat[]);
    } catch (error) {
  ...
  const saveAboutContent = async () => {
    ...
    try {
      const result = await updateAboutContent(aboutContent.id, {
      ...
      if (!result.success) throw new Error('Gagal menyimpan about content');
  ...
  const saveAllStats = async () => {
    ...
    try {
      const result = await updateHomeStats(stats);

      if (!result.success) throw new Error('Gagal menyimpan stats');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <ImageInputWithUpload
              ...
              onChange={(url) => setAboutContent(aboutContent ? { ...aboutContent, lanyard_texture_file: url } : null)}
              folder="lanyard"
              accept="image/png"
              previewSize="md"
              placeholder="/assets/lanyard/lanyard.png"
            />
            <p className="mt-1 text-xs text-gray-500">Upload gambar PNG atau masukkan URL/path file.</p>
          </div>
        </div>
