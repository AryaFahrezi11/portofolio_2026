...
import { getSkills, addSkill, updateSkill, deleteSkill, toggleSkillActive } from '@/app/actions/portfolio';
...
export default function AdminSkills() {
  ...
  const handleAddSkill = async () => {
    ...
    try {
      ...
      if (!result.success) throw new Error('Gagal menambahkan skill');
  ...
  const handleUpdateSkill = async (skill: Skill) => {
    setIsSaving(true);
    try {
      const result = await updateSkill(skill.id, skill);

      if (!result.success) throw new Error('Gagal mengupdate skill');
  ...
  const handleDeleteSkill = async (id: number) => {
    ...
    try {
      ...
      if (!result.success) throw new Error('Gagal menghapus skill');
  ...
  const toggleActive = async (skill: Skill) => {
    try {
      const result = await toggleSkillActive(skill.id, !skill.is_active);
