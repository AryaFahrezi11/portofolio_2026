...
import { updateAdminProfile } from '@/app/actions/portfolio';
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      const result = await updateAdminProfile({
        oldUsername: editForm.oldUsername || adminUsername,
        oldPasswordPlain: editForm.oldPassword,
        newUsername: editForm.newUsername || undefined,
        newPasswordPlain: editForm.newPassword || undefined,
      ...
      if (editForm.newUsername) {
        localStorage.setItem('admin_username', editForm.newUsername);
        setAdminUsername(editForm.newUsername);
      }
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                href={item.path}
                className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg transition-all ${isActive
                  ...
                  <div>
                    <ImageInputWithUpload
                      ...
                      onChange={(url) => {
                        ...
                        // Hide manual preview since component has its own
                        setPhotoPreview('');
                      }}
                      folder="avatars"
                      previewSize="lg"
                    ...
                    <p className="text-gray-500 text-xs mt-1">
                      Upload foto baru atau gunakan URL gambar yang sudah ada.
                    </p>
                  </div>
                </div>
              </div>
