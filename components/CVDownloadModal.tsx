...
const CVDownloadModal = ({ isOpen, onClose, cvUrl }: CVDownloadModalProps) => {
    ...
    const handleDownload = async (e: React.MouseEvent<HTMLAnchorElement>, url: string, filename: string) => {
        // Prevent default only if we want to handle it via JS
        // We'll try to fetch it to force download if it's an external URL
        if (url.startsWith('http')) {
            ...
            try {
                ...
                // Fallback to normal link behavior if fetch fails
                                        ...
                                        <>
                                            ...
                                            >
                                                ...
                                                <div className="relative bg-[#0f0f0f] hover:bg-black/80 text-white rounded-[11px] px-6 py-3.5 transition-colors flex items-center justify-center gap-2 font-medium">
                                                    <span>Download CV {cvUrl.toLowerCase().endsWith('.pdf') ? '(PDF)' : ''}</span>
                                                    <svg className="w-5 h-5 group-hover:translate-y-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                                    </svg>
                                                </div>
                                            </a>
                                            {/* Informational message if not PDF */}
                                            {!cvUrl.toLowerCase().endsWith('.pdf') && (
                                                <p className="text-[10px] text-white/40 italic">File format: {cvUrl.split('.').pop()?.toUpperCase()}</p>
                                            )}
                                        </>
                                    ) : (
                                        <p className="text-white/40 italic text-sm">Maaf, file CV belum tersedia.</p>
                                    )}
                                </div>
